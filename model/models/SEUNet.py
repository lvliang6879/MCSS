...
from model.models.basic import _ConvBNReLU
# from att import *
...
class SENet(nn.Module):
    ...
    def decoder_ConvTranspose(self, x1, x2, x3, x4, x5, refined_features=None):
        ...
        if refined_features != None:
            ...
            prototype_predict = torch.zeros_like(seg_out)
            for refined_feature in refined_features:
                refined_feature = F.interpolate(refined_feature, (seg_out.shape[2], seg_out.shape[3]), mode='bilinear')
                # prototype_predict.append(refined_feature)
                prototype_predict += refined_feature  #first
                seg_out += refined_feature
                # seg_out += F.interpolate(refined_feature, (seg_out.shape[2], seg_out.shape[3]), mode='bilinear')
            # seg_out += prototype_predict[0]
            return seg_out, prototype_predict
        else:
            seg_out = self.classifier(score)
            return seg_out
    ...
    def forward(self, x, class_features = None):
        ...
        if class_features != None:
            ...
            refined_features = self.refine_features(features, class_features.squeeze(0))
